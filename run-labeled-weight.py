...
def calcMergeSimMatrix(simMatrixDict, labelList):
    ...
    # New !! 重み付け
    for key in simMatrixDict:
        weightsDict = openWeights("weight_{}.txt".format(key))
        for i, row in enumerate(simMatrixDict[key]):
            weighted_data = [applyWeights(value, weightsDict)
                             for value in row]
            simMatrixDict[key][i] = weighted_data
...
def openWeights(filePath):
    with open(filePath, "r") as f:
        data = {}
        for line in f:
            values = line.strip().split(", ")
            x_value = float(values[0].split(": ")[1])
            weight_value = float(values[1].split(": ")[1])
            data[x_value] = weight_value

    return data


def applyWeights(value, weights):
    closest_x = min(weights.keys(), key=lambda x: abs(x - value))
    return value * weights[closest_x]
