...
def main():
    ...
    print("Negative数の標準偏差: ", statistics.pstdev(negativeCountList))
    ...
    print("HardNegative数の標準偏差: ", statistics.pstdev(hardNegativeCountList))
    ...
    print("easyNegative数の標準偏差: ", statistics.pstdev(easyNegativeCountList))
    
    exit()
    # 出力
    dataClassList = ["train", "dev"]
    for key in tripletDataDictOfOriginal:
        trainList, devList = train_test_split(tripletDataDictOfOriginal[key], test_size=0.05, shuffle=True, random_state=314)    
    ...
    if not isOnlyMethod1:
        for key in tripletDataDictOfSpecterPaper:
            trainList, devList = train_test_split(tripletDataDictOfSpecterPaper[key], test_size=0.05, shuffle=True, random_state=314)    
            for dataClass in dataClassList:
        ...
        for key in tripletDataDict2:
            trainList, devList = train_test_split(tripletDataDict2[key], test_size=0.05, shuffle=True, random_state=314)    
            for dataClass in dataClassList:
        ...
        for key in tripletDataDict3:
            trainList, devList = train_test_split(tripletDataDict3[key], test_size=0.05, shuffle=True, random_state=314)    
            for dataClass in dataClassList:
