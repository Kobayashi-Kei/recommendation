...
"""
関連研究推薦の実験
・分類されたアブストラクトのすべての観点同士で比較して和/平均を取る
"""	
def main():
...
Class & Methods
"""
def calcSimMatrixForLaveled(allPaperData: allPaperDataClass, testPaperData: PaperDataClass, labelList, vectorizer=None):
    # 背景、手法などの類似度を計算した行列を格納する辞書
    simMatrixDict = {v : np.zeros((len(testPaperData.paperList), len(allPaperData.paperList))) for v in labelList}
    ...
    for key in labelList:
        isNotNoneMatrixDict[key] = np.ones((len(testPaperData.paperList), len(allPaperData.paperList)))        
    labeledVectorDict = {}
    for key in labelList:
        if vectorizer:
            tmpVectorList = []
            ...
            for i, vector in enumerate(tmpVectorList):
                ...
                if vector == None:
                    ...
                    if i in testPaperData.allDataIndex:
                        isNotNoneMatrixDict[key][allPaperData.testDataIndex[i],:] = 0
        labeledVectorDict[key] = tmpVectorList
    
    # クエリ論文のそれぞれのラベルのベクトルを抽出
    for key in labeledVectorDict:
        testPaperVectorList = extractTestPaperEmbeddings(
                labeledVectorDict[key], 
        ...
        # 1つのラベルのベクトルと全てのラベルに関するベクトルを比較
        for key_2 in labeledVectorDict:
            # TF-IDFやBOWの場合は疎行列となるため、csr_sparse_matrixに変換して速度を上げる
            if vectorizer:
                tmpVectorList = csr_matrix(labeledVectorDict[key_2])
            else:
                tmpVectorList = labeledVectorDict[key_2]
            ...
            simMatrix = simMatrix*isNotNoneMatrixDict[key]
            
            # 異なるラベル同士の比較なら、重みをかけて減らす
            hparam = 0.03
            if key != key_2:
                if not (key == 'bg' and key_2 == 'obj') and not (key == 'obj' and key_2 == 'bg'):
                    simMatrix = simMatrix * hparam
                else:
                    simMatrix = simMatrix * hparam
                    
            simMatrixDict[key] += simMatrix
            
        # print(simMatrixDict[key])
        # print(key)
        # exit()
        simMatrixDict[key] = np.where(simMatrixDict[key]==0, np.nan, simMatrixDict[key])
        # print(simMatrixDict[key])
    # print(simMatrixDict)
